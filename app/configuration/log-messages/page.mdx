...
When a message is logged, the log entry includes an inline **mute button**. This allows moderators to quickly mute the user who sent the blocked message directly from the log channel, without needing to use the `/mute-user-from-interserv-group` command.
...
The log channel is configured from the [dashboard](https://oraxbot.com/dashboard). `/manage-group` only exposes toggle settings.

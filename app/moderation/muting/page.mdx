...
Use `/unmute-user-from-interserv-group` with a message link or user ID, or unmute from the dashboard.

### Silent mute

Pass `silent:true` on `/mute-user-from-interserv-group` to leave the user's messages visible in the origin channel while blocking relay.

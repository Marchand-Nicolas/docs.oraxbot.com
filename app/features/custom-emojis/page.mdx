...
1. A user types `:!emoji_name:` in a linked channel (on Discord or Fluxer)
2. Orax scans the message for the `:!...:` pattern
3. The bot searches for an emoji with that name across all servers it's in
4. If found, the `:!emoji_name:` is replaced with the actual emoji markup
...
## Cross-Platform Emojis

Emojis also work **across platforms**. When a message containing custom emoji markup (like `<:peepo:123456>`) is relayed to another platform or a server that doesn't have that emoji, Orax translates it automatically:

1. If the destination server already has an emoji with the **same name**, it's swapped in
2. Otherwise, the emoji is converted to a small **linked image** (`[:peepo:](image-url)`) hosted on the origin platform's CDN

When resolving an emoji by name, Orax searches in this order:

1. The **destination server's** emojis
2. Emojis from other servers in the **interserv group**
3. Any server the bot is in

This translation also applies when messages are [edited](/features/edit-delete-sync) — emojis are re-resolved for the destination platform.

...
<Callout type="warning">
  Linking a voice channel requires **Orax Plus** — either on your server or on
  the server that owns the group. See [Orax Plus](/premium) for how to get it
  (including the free vote option).
</Callout>
...
- **Fluxer**: run `/link-channel <group_link>` in the voice channel, or pick it from the [dashboard](https://oraxbot.com/dashboard/fluxer)
...
<Callout type="info">
  Discord voice messages (audio notes) are excluded from edit sync, since they
  can never be edited.
</Callout>
...
<Callout type="info">
  On Fluxer, voice channels using the **legacy UDP protocol** are send-only:
  Orax can relay audio *into* them, but audio from Fluxer participants is not
  captured. Channels on the LiveKit backend (the default) support full two-way
  audio.
</Callout>
